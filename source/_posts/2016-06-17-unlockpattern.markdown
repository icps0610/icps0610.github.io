...
def check(num)
    (0..num.length-2).each do |i|
        ...
        n = num[i+1]
        co = "1379"
        if co.include?(l.to_s) and co.include?(n) and not num.include?("#{(l.to_i + n.to_i)/2}")
            return false
        elsif n.to_i + l.to_i == 10  and not num.include?("#{(l.to_i + n.to_i)/2}")
            return false
        end
    end
    return true
