...
func printError(err error, es ...string) {
    if err != nil {
...
func writeJson(data []Image, path string) {
...
###### unix
``` go
func unixTime(y, m, d, h, mi int) int {
...
###### string <-> to_float
``` go
fn, _  := strconv.ParseFloat(cell, 64
```
###### fill_zero
``` go
...
func openBrowser() (*agouti.Page, *agouti.WebDriver) {
    driver := agouti.ChromeDriver(
        // agouti.ChromeOptions("args", []string{"--window-size=1980,1020", "headless"}),
        agouti.ChromeOptions("args", []string{"--window-size=1980,1020"}),
    )
    err := driver.Start()
    printError(err)
